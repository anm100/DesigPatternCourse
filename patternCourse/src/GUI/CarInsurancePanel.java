...
public class CarInsurancePanel extends JPanel implements InterfaceComposite{
	...
	private CarInsurancePanel thisref=this;
	private ArrayList<Insurance> apps_list= new  ArrayList<Insurance>();
	private ArrayList<CarInsurance> data;
	CarInsuranceController cont;
	 ...
	 */
	public CarInsurancePanel(MainScreen mainScreen) {
	...
	private void initialize() {
		...
		panel.add(lblPhone);
				...
				JLabel logo = new JLabel("car Insurance");
		 ...
		 cont=new CarInsuranceController(thisref);
...
//"sid","Policy Number","Client ID","Company insurance","start date","expiration date","price"};
	private void initData() {
		...
		if (data != null) {
			...
			if (apps_list != null && apps_list.size() > 0)
				apps_list.clear();
			for (CarInsurance  a : data) {
				dm.addRow(new Object[] { a.getSid(),a.getNumberofpolicy()
						,a.getClientID(),a.getCarNumber(),a.getCompanyInsurance(),a.getStartDate(),a.getEndDate(), a.getPrice()});
				apps_list.add(a);
			}
		}
	}
	...
	public void setdata(ArrayList<CarInsurance> list){
