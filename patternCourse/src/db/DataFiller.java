...
public class DataFiller {
	...
	public void fillconfirmation() throws Exception {
		 int count=0; 
		 int j=0; 
		ArrayList <Param> patients=  new ArrayList <Param> (); 
		
		patients=(ArrayList<Param>) db.patients.queryForAll();

		for (int i=0 ; i < patients.size(); i++ ){
			
			Param p = patients.get(i);
			/// referral 1 
	...
	public void fillAppointments() throws SQLException, ParseException {
		...
		ArrayList<Param> patients= (ArrayList<Param>) db.patients.queryForAll();
		if(patients==null || patients.size()==0) return;
		
		for(Param patient: patients){
